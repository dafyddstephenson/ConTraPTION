...
MODULE domwri
   ...
   PUBLIC dom_uniq       ! required for NEMOTAM
...
CONTAINS
   ...
   SUBROUTINE dom_uniq( puniq, cdgrd, kindic )
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE dom_uniq  ***
      !!                   
      ...
      !
      IF ( PRESENT( kindic ) ) THEN
         puniq(:,:) = 0.0_wp                      ! for generating interior mask
      ELSE
         puniq(:,:) = 1.0_wp                      ! default definition
      ENDIF
