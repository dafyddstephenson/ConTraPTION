MODULE step_tam
   ...
   !!----------------------------------------------------------------------
        ...
        USE sbcssr_tam, ONLY : nn_sstr, &
             & qrp_tl, qrp_ad,  &
             & erp_tl, erp_ad            !!! 2016-07-25 added as nn_str did not have assigned type

   USE step_oce_tam
#if defined key_agrif
#error 'agrif not yet implemented in nemotam'
#endif
   ! 2016-06-22 /attempted/ to add an option to remove and store top-layer tracer before advancing
...
CONTAINS
   SUBROUTINE stp_tan( kstp )
      ...
      !                          DO jj = 1, jpj
     !                              DO ji = 1, jpi
    !                                  qrp_tl(ji,jj) = rn_dqdt * (tsn_tl(ji,jj,1,jp_tem) - 1.0_wp)
   !                                   qns_tl(ji,jj) = qns_tl(ji,jj) +  qrp_tl(ji,jj)
  !                                 END DO
 !                               END DO
!                             ENDIF
!!! 2016-22-08 Added section to restore EDW to 1 and non-EDW to 0 at surface:
                             IF (ln_edw_auto) THEN

                                ! 2016-08-23 added safety catch
                                IF (nn_sstr.EQ.1) THEN
                                   CALL ctl_stop('Incompatible tracer-removal schemes!')
                                END IF
                               
                                qrp_tl(:,:) = 0._wp
!!!                                WHERE (tmsk_region(:,:,1)*tmsk_nasmw(:,:,1) > 1e-6_wp)
                                   !qrp_tl(:,:) = rn_dqdt * (tsn_tl(:,:,1,jp_tem) - 1.0_wp)
!!!                                   qrp_tl(:,:) =  -40._wp * (tsn_tl(:,:,1,jp_tem) - 1.0_wp)
!!!                                   qns_tl(:,:) = qns_tl(:,:) +  qrp_tl(:,:)
!!!                                END WHERE
                                
                                tmp_rm_edw(:,:) = tmp_rm_edw(:,:) - qrp_tl(:,:)*rdttra(1)*ro0cpr*e1t(:,:)*e2t(:,:)
                                qrp_tl(:,:) = 0._wp                                
!!!                                WHERE (tmsk_region(:,:,1)*tmsk_nasmw(:,:,1) <1e-6_wp)
!                                  qrp_tl(:,:) = rn_dqdt * tsn_tl(:,:,1,jp_tem)
!!!                                   qrp_tl(:,:) = -40._wp * tsn_tl(:,:,1,jp_tem)
!!!                                   qns_tl(:,:) = qns_tl(:,:) +  qrp_tl(:,:)
!!!                                END WHERE
                             ...
                             END IF
!!!2016-22-08 end of modifications
      ...
      IF( ln_tradmp      )   CALL tra_dmp_tan( kstp )       ! internal damping trends
                             CALL tra_adv_tan( kstp )       ! horizontal & vertical advection !!!2016-08-05: commented call to advection to run with diffusion only
                                                                                              !!!2016-08-07 uncommented again
...
!2016-06-22 added storage of top-layer tracer followed by removal

           ! 2016-07-21 diagnose removed tracer if nn_sstr is 1
           IF (nn_sstr == 1) THEN
               tmp_rm(:,:) = tmp_rm(:,:) - qrp_tl(:,:)*rdttra(1)*ro0cpr*e1t(:,:)*e2t(:,:)
           END IF

!2016-07-01 remove all negative tracer

           IF (ln_tl_surf_remove) THEN 

              ! 2016-07-21 added stop
              IF (nn_sstr == 1) THEN
                 CALL ctl_stop('Incompatible tracer-removal diagnostics selected...')
              END IF

!              WHERE (tsn_tl(:,:,:,jp_tem) <= 0.0_wp)
!                 tsn_tl(:,:,:,jp_tem) = 0.0_wp;
!              END WHERE  !!!removed until further notice.

  
!    	       tra_rm(:,:) = tra_rm(:,:) + tsn_tl(:,:,1,jp_tem)*(e1t(:,:)*e2t(:,:)*(fse3t(:,:,1)+sshn(:,:))) !!2016-06-27
               tmp_rm(:,:) = tmp_rm(:,:) + tsn_tl(:,:,1,jp_tem)*( (fse3t(:,:,1)+sshn(:,:))/fse3t(:,:,1)) !!2016-06-28               
               sal_rm(:,:) = sal_rm(:,:) + tsn_tl(:,:,1,jp_sal)*( (fse3t(:,:,1)+sshn(:,:))/fse3t(:,:,1)) !!2016-06-28
               nn_tra_rm = nn_tra_rm + 1
               tsn_tl(:,:,1,:) = 0.0_wp
               tsb_tl(:,:,1,:) = 0.0_wp
           END IF
! moved up 2016-06-28

            ! 2016-06-21 moved on-line resetting of NASMW from nemogcm_tam.F90 to here, and changed 'istp' to 'kstp' and 'ztn_tlin' to 'tmsk_region'
            ! 2016-06-09 added switch and frequency for on-line resetting of NASMW
            IF (ln_tl_nasmw_auto) THEN
               IF (MOD(kstp-nit000+1, nn_tl_nasmw_freq).EQ.0) THEN
                  WHERE ((tsn(:,:,:,jp_tem) >= 17.0_wp).AND.(tsn(:,:,:,jp_tem) <= 19.0_wp))
!!!                     tsn_tl(:,:,:,jp_tem) = tmsk_region(:,:,:)
!!!                     tsn_tl(:,:,:,jp_sal) = tmsk_region(:,:,:)
                  END WHERE
               END IF
            END IF
...
!*B This fails with cgmod. To be revised      CALL stp_ctl_tan( kstp, indic, 0 )
   ...
   SUBROUTINE stp_adj( kstp )
      ...
      ! ... Read basic state trajectory at end of previous step
                             ...
                             ! 2016-06-20: added output of adjoint state
           ...
           IF (ln_tl_surf_remove) THEN  
              ...
              END IF
               ...
               tmp_rm(:,:) = tmp_rm(:,:) + tsn_ad(:,:,1,jp_tem)*( (fse3t(:,:,1)+sshn(:,:))/fse3t(:,:,1)) !!2016-06-28               
               sal_rm(:,:) = sal_rm(:,:) + tsn_ad(:,:,1,jp_sal)*( (fse3t(:,:,1)+sshn(:,:))/fse3t(:,:,1)) !!2016-06-28
               nn_tra_rm = nn_tra_rm + 1
               tsn_ad(:,:,1,:) = 0.0_wp
               tsb_ad(:,:,1,:) = 0.0_wp
           END IF

           ! 2016-07-21 Diagnostics of removed tracer if nn_sstr is 1 can be found in module sbcmod_tam
      ...
      ! vertical mesh at next time step ... not available
                             ...
                             ! 2016-06-20 added switch for passive tracer transport in adjoint version of time step; 2016-07-29 corrected *_tl -> *_ad
                             IF (ln_tl_passive_tracer) THEN
                                sshb_ad(:,:) = 0.0_wp
                                sshn_ad(:,:) = 0.0_wp
                             ENDIF
      ...
      ! penetrative convective adjustment ... not available
                             ...
                             CALL tra_adv_adj( kstp )      ! horizontal & vertical advection !!!2016-08-05: removed call to advection to run with diffusion only
                                                                                             !!!2016-08-07: uncommented again
