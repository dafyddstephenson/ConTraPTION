...
#  include "domzgr_substitute.h90"
  ...
  REAL(wp), ALLOCATABLE, DIMENSION(:,:,:), SAVE :: &
      ...
      & avsr2,    &
      & etot3r2
! 2016-03-03 Added 'sshn' to trajectory
  REAL(wp), ALLOCATABLE, DIMENSION(:,:), SAVE :: &
      & hmlp1,    &
      & hmlp2,    &
      & sshnr1,   &
   ...
   SUBROUTINE trj_rea( kstp, kdir )
	 ...
	 & it,     &
      ...
      REAL(KIND=wp) :: &
         ...
         & zden,  &
         & zstp
      REAL(KIND=wp),dimension(jpi,jpj) :: z_edwthickness
     
      ! Initialize data and open file
      !! if step time is corresponding to a saved state
      ! 2016-07-19 multiplication factor for reading/writing trajectory
      IF ( ( MOD( kstp - nit000 + 1, nn_ittrjfrq*nn_dtfactor ) == 0 )  ) THEN

         ! 2016-06-03 added offset to trajectory file labels
         !it = kstp - nit000 + 1
         ! 2016-07-19 multiplication factor for reading/writing trajectory
         it = ( kstp - nit000 + 1)/nn_dtfactor  + nn_ittrjoffset

         IF ( inumtrj1 == -1 ) THEN

            ! Define the input file
	    ! 2016-02-29 - modified output filenames
            !WRITE(cl_dirtrj, FMT='(I5.5,A,A,".nc")' ) it, '_', TRIM( cn_dirtrj )
      	    ! 2016-06-03 - extended number of digits in trajectory file labels
            !WRITE(cl_dirtrj, FMT='(A,A,I5.5,".nc")' ) TRIM( cn_dirtrj ), '_', it
...
! 2016-03-03 Added 'sshn' to trajectory
      ...
      ! Read records
         ...
         ! 2016-07-19 multiplication factor for reading/writing trajectory
         inrcm = INT( ( kstp - nit000 + 1 ) / (nn_ittrjfrq*nn_dtfactor) ) + 1
...
! 2016-03-03 Added 'sshn' to trajectory
	    ...
	    ! 2016-02-29 - modified output filenames
            !WRITE(cl_dirtrj, FMT='(I5.5,A,A,".nc")' ) (it-nn_ittrjfrq), '_', TRIM(cn_dirtrj )
      	    ! 2016-06-03 - extended number of digits in trajectory file labels
            !WRITE(cl_dirtrj, FMT='(A,A,I5.5,".nc")' ) TRIM(cn_dirtrj ), '_', (it-nn_ittrjfrq)
...
!            inrc = inrcm
            ...
            inrc = 1
            ! 2016-07-19 multiplication factor for reading/writing trajectory
            stpr1 = (inrcm - 1) * nn_ittrjfrq*nn_dtfactor
...
! 2016-03-03 Added 'sshn' to trajectory
         ...
         IF ( ( ( kstp /= nitend ) .AND. ( kdir == 1  )) .OR. &
            ...
            ! 2016-07-19 multiplication factor for reading/writing trajectory
            stpr2 = (inrcp - 1) * nn_ittrjfrq*nn_dtfactor
...
! 2016-03-03 Added 'sshn' to trajectory
      ...
      ! 2016-07-19 multiplication factor for reading/writing trajectory
      IF ( (kstp == nitend) .AND. ( MOD( kstp - nit000 + 1, nn_ittrjfrq*nn_dtfactor ) /= 0 )  ) THEN
          IF(lwp) WRITE(numout,*) '   Warning ! nitend (=',nitend, ')', &
               &                  ' and saving frequency (=',nn_ittrjfrq*nn_dtfactor,') not compatible.'
      ENDIF
...
! 2016-05-18 Added 'sshb' to trajectory
         sshb(:,:)     = zwtr1 * sshnr1    (:,:) + zwtr2 * sshnr2    (:,:)
! 2016-05-18 - SAM: adjusted outputting of interpolation coefficients
...
!!!2016-08-19 create mask of EDW where T in (17,19) and thickness > 125 m
!tmsk_nasmw(:,:,:)   = 0._wp
z_edwthickness(:,:) = 0._wp

DO jk = 1,jpk
   WHERE ((tsn(:,:,jk,jp_tem)<19.0_wp) .AND. (tsn(:,:,jk,jp_tem)>17.0_wp))
      z_edwthickness(:,:) = z_edwthickness(:,:) + fse3t(:,:,jk)
!      tmsk_nasmw(:,:,jk)  = 1.0_wp
   END WHERE
END DO

z_edwthickness(:,:) = z_edwthickness(:,:) + sshn(:,:)


DO jk = 1,jpk  
   WHERE (z_edwthickness(:,:) < 125._wp)
!      tmsk_nasmw(:,:,jk) = 0._wp
   END WHERE
END DO

avt(:,:,:) = avt(:,:,:) * rn_avtfactor !!! 2016-08-23 rescaling vertical diffusivity by fixed factor
   ...
   ! 2016-06-20: added switch for alternative writing of adjoint state
   SUBROUTINE tl_trj_wri(kstp, kadj)
      ...
      IF ( ( MOD( kstp - nit000 + 1, nn_ittrjfrq_tan ) == 0 )  ) THEN
         ...
         it = kstp - nit000 + 1
            ...
            ! Define the input file
            ! 2016-03-03 - modified output filenames
            ! 2016-06-03 - extended number of digits in trajectory file labels
            !WRITE(cl_tantrj, FMT='(A,A,I5.5,".nc")' ) TRIM( cn_tantrj ), '_', it
            WRITE(cl_tantrj, FMT='(A,A,I6.6,".nc")' ) TRIM( cn_tantrj ), '_', it
            ...
            ! Output trajectory fields
            ! 2016-06-20: added alternative writing of adjoint state
            !             instead of tangent-linear state
            ...
            ELSE
               ...
               CALL iom_rstput(it, it, inum, 'tb_ad',    tsb_ad(:,:,:,jp_tem))
            ...
            CALL iom_rstput( it, it, inum, 'wn'   , wn   ) ! 2016-06-28
            CALL iom_rstput( it, it, inum, 'sshn' , sshn )
            CALL iom_rstput( it, it, inum, 'tmp_rm' , tmp_rm(:,:) )
            CALL iom_rstput( it, it, inum, 'tmp_rm_edw' , tmp_rm_edw(:,:) ) !!!2016-08-22
            CALL iom_rstput( it, it, inum, 'sal_rm' , sal_rm(:,:) )      

               tmp_rm(:,:) = 0.0_wp ! reset cumulative total after output
               tmp_rm_edw(:,:)=0.0_wp! 2016-08-22
               nn_tra_rm = 0        ! reset timestep counter
               sal_rm(:,:)  = 0.0_wp
            CALL iom_close( inum )
