MODULE oce_tam
   ...
   !! 2016-06-22 added tra_rm variable which stores the mean quantity of tracer in the surface layer between outputs before removing it
   !! 2016-06-28 renamed tra_rm to tmp_rm and added sal_rm to distinguish tracers
   ...
   !
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)         ::            tmp_rm,sal_rm,tmp_rm_edw            !: mean tracer volume removed in top layer !!!2016-22-08 added tmp_rm_edw
   INTEGER, PUBLIC, SAVE                                       ::            nn_tra_rm
   !
...
CONTAINS
   INTEGER FUNCTION oce_alloc_tam( kmode )
      ...
      ! 2016-07-29 allocate irrespective of value of jmode
      IF (.NOT.ALLOCATED(tmp_rm)) THEN
         ALLOCATE( &
              & tmp_rm(jpi,jpj) ,	sal_rm(jpi,jpj),      & 				!!! added 2016-06-22/28
              & tmp_rm_edw(jpi,jpj), &                                                          !!! added 2016-08-22
              & STAT=ierr(1))
      END IF
   ...
   INTEGER FUNCTION oce_dealloc_tam( kmode )
      ...
      ! 2016-07-29 allocate irrespective of value of jmode
      IF (ALLOCATED(tmp_rm)) THEN
         DEALLOCATE( &
!!!!!	    &			      tmp_rm , sal_rm, 		 &			!!!added 2016-06-22/28
	    &       tmp_rm      , sal_rm    ,   		   &			!!!added 2016-06-22/28
            &       tmp_rm_edw  ,                                  &                    !!!added 2016-08-22
            & STAT=ierr(1))
      END IF
   ...
   SUBROUTINE oce_tam_init( kmode )
      ...
      INTEGER :: ierr
      ! 2016-07-29 initialise irrespective of value of kmode
      tmp_rm(:,:)    = 0._wp
      sal_rm(:,:)    = 0._wp
      tmp_rm_edw(:,:)= 0._wp !!!2016-08-22
