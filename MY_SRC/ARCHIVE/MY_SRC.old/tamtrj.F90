MODULE tamtrj
   ...
   INTEGER, PUBLIC :: nn_ittrjoffset

   ! 2016-07-19 multiplication factor for reading/writing trajectory
   INTEGER, PUBLIC :: nn_dtfactor
      REAL, PUBLIC :: rn_avtfactor !!!2016-08-22
...
CONTAINS
   SUBROUTINE tam_trj_init
      ...
      ! 2016-06-03 added offset to trajectory file labels
      ! 2016-06-06 corrected namelist name
      NAMELIST/namtrj/ nn_ittrjoffset
      ! 2016-07-19 multiplication factor for reading/writing trajectory
      NAMELIST/namtrj/nn_dtfactor
      NAMELIST/namtrj/rn_avtfactor !2016-08-23 rescaling vert. diff.
	 ...
	 ! 2016-06-03 added offset to trajectory file labels
	 WRITE(numout,*) '             Offset for trajectory file labels'// &
	    &            ' nn_ittrjoffset = ', nn_ittrjoffset
         ! 2016-07-19 multiplication factor for reading/writing trajectory
         WRITE(numout,*) '            multiplication factor of trajectory filenames            ', &
            &            ' nn_dtfactor =   ', nn_dtfactor
      END IF
   ...
   SUBROUTINE tam_trj_wri( kt )
      ...
      !------------------------------------------------------------------------
      ! 2016-07-19 multiplication factor for reading/writing trajectory
      IF( ( MOD( kt - nit000 + 1, nn_ittrjfrq*nn_dtfactor ) == 0 ) .OR. ( kt == nitend ) ) THEN
	 ...
	 ! 2016-06-03 - extended number of digits in trajectory file labels
         ...
         WRITE(cl_dirtrj, FMT='(A,A,I6.6)' ) TRIM( cn_dirtrj ), '_', it 
