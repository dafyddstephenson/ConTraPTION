...
!RBbug do not understand why see ticket 667
      ...
      ! 2016-07-21 Diagnostics of removed tracer if nn_sstr is 1 can be found in module sbcmod_tam (deactivated); 2016-07-29 activated, replaced qrp_ad by sst_m_ad (?)
      IF (nn_sstr == 1) THEN
         tmp_rm(:,:) = tmp_rm(:,:) - sst_m_ad(:,:)!*rdttra(1)*ro0cpr*e1t(:,:)*e2t(:,:)
      END IF
