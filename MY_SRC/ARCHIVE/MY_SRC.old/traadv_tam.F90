MODULE traadv_tam
   ...
   USE wrk_nemo
   USE timing
   ! 2016-05-16 Addition of EIV to background trajectory (optional)
   USE traadv_eiv
   USE tamctl
   ...
   LOGICAL, PUBLIC :: ln_traadv_tvdpt = .FALSE. ! 2016-11-29 added TVD advection scheme as an option for passive tracer transport 
...
CONTAINS
   ...
   SUBROUTINE tra_adv_tan( kt )
      ...
      ! 2016-05-16 Addition of EIV to background trajectory (optional)
      IF (ln_tl_eiv) THEN
      ! 2016-05-16 Addition of EIV to background trajectory (optional); the next two lines are a copy of the corresponding call from file OPA_SRC/TRA/traadv.F90 (lines 96 and 97)
      ...
      IF ( kt == nit000 ) THEN
         ...
         IF (ln_traadv_tvdpt) THEN
            if (lwp) write(numout,*) 'TVD advection scheme in use for passive tracer transport'
            nadv = 2
         ELSE
...
! 2016-06-21: trajectory-upstream advection inactive; 2016-07-05 re-enabled
         CALL tra_adv_cen2_tan ( kt, nit000, zun, zvn, zwn, tsn, zuntl, zvntl, zwntl, tsn_tl, tsa_tl, jpts, rn_tl_adv_ups )    ! 2nd order centered scheme
...
! 2016-06-21: trajectory-upstream advection inactive; 2016-07-05 re-enabled
      CASE ( 1 ) ; CALL tra_adv_cen2_adj ( kt, nit000, zun, zvn, zwn, tsn, zunad, zvnad, zwnad, tsn_ad, tsa_ad, jpts, rn_tl_adv_ups )    ! 2nd order centered scheme
      END SELECT
      !
      DO jk = jpkm1, 1, -1
...
! 2016-11-29 reading in of namelist namtra_adv instead of not reading in namelist nam_traadv
!      NAMELIST/nam_traadv/ ln_traadv_cen2 , ln_traadv_tvd,    &
      NAMELIST/namtra_adv/ ln_traadv_cen2 , ln_traadv_tvd,    &
         ...
         &                 ln_traadv_tvdpt ! 2016-11-29 added TVD advection scheme as an option for passive tracer transport 
      !!----------------------------------------------------------------------
...
!      READ   ( numnam, nam_traadv )
      ...
      IF(lwp) THEN                    ! Namelist print
         ...
         WRITE(numout,*) ' TVD advection scheme for passive tracer transport = ', ln_traadv_tvdpt ! 2016-11-29 added TVD advection scheme as an option for passive tracer transport 
