...
!!!later         END IF
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpjm1
               DO ji = 1, jpim1
                  zthtu(ji,jj,jk) = ( pun(ji,jj,jk) / (e2t(ji,jj)*e3t(ji,jj,jk)) ) / ( 2.0_wp * (1e-19_wp+rldf * ahtu(ji,jj)))
                  zthtv(ji,jj,jk) = ( pvn(ji,jj,jk) / (e1t(ji,jj)*e3t(ji,jj,jk)) ) / ( 2.0_wp * (1e-19_wp+rldf * ahtv(ji,jj)))
               END DO
            END DO
         END DO
